    ...
    if (eachRC._id.$oid === eachC.rootCategoryId.$oid) {
      ...
      for (let i in subCategories) {
        ...
        if (eachC._id.$oid === eachSC.categoryId.$oid) {
          console.log(
            "Matched For Category ID : " +
              eachC.name +
              "Category ID : " +
              eachSC.name
          );
          eachTxSubCat = eachSC;
          tranformedSubCategories.push(eachTxSubCat);
        }
      }
      tranformedCategories.name = eachC.name;
      tranformedCategories.description = eachC.description;
      tranformedCategories.subcategories = tranformedSubCategories;
      second_level_result.push(tranformedCategories);
      //   }// Close of for
      eachTxCat = eachC;
      eachTxCat.subcategories = second_level_result;
      // tranformedCategories.subcategories = eachTxCat;
    }
  }
...
console.log("first_level_result : " + JSON.stringify(first_level_result));
/*let second_level_result = [];
let tranformedSubCategories = [];
...
for (let i in categories) {
  ...
  tranformedCategories.name = eachCategory.name;
  tranformedCategories.description = eachCategory.description;
  tranformedCategories.subcategories = tranformedSubCategories;
  second_level_result.push(tranformedCategories);
}
