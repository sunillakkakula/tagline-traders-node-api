...
let first_level_result = [];
for (let i in rootCategories) {
  let tranformedRootCategories = {};
  let eachRC = rootCategories[i];
  let tranformedCategories = [];
  /**
   * ADDING second_level_result []
   */
  let second_level_result = [];
  let tranformedSubCategories = [];
  for (let i in categories) {
    let eachC = categories[i];
    if (eachRC._id.$oid === eachC.rootCategoryId.$oid) {
      /**
       *  NOW U R IN EACH CATEGORY ADDING SUB CAT LOGIC START
       */

      for (let i in subCategories) {
        let eachSC = subCategories[i];
        if (eachC._id.$oid === eachSC.categoryId.$oid) {
          tranformedSubCategories.push(eachSC);
          tranformedCategoriesSecondLevel.name = eachSC.name;
          tranformedCategoriesSecondLevel.description = eachSC.description;
          // tranformedCategoriesSecondLevel.subcategories = tranformedSubCategories;
        }
      }
      // tranformedCategoriesSecondLevel.subcategories = tranformedSubCategories;
      // tranformedCategoriesSecondLevel.name = eachC.name;
      // tranformedCategoriesSecondLevel.description = eachC.description;
      tranformedCategoriesSecondLevel.subcategories = tranformedSubCategories;
      second_level_result.push(tranformedSubCategories);
      /**
       *  NOW U R IN EACH CATEGORY ADDING SUB CAT LOGIC END
       */
      eachC.subCategories = second_level_result;
      tranformedCategories.push(second_level_result);
    }
  }
  tranformedRootCategories.name = eachRC.name;
  tranformedRootCategories.description = eachRC.description;
  tranformedRootCategories.categories = tranformedCategories;
  first_level_result.push(tranformedRootCategories);
}

console.log("first_level_result : " + first_level_result);
console.log("end");
// let second_level_result = [];

// for (let i in categories) {
//   let tranformedCategoriesSecondLevel = {};
//   let eachCategory = categories[i];
//   let tranformedSubCategories = [];
//   for (let i in subCategories) {
//     let eachSC = subCategories[i];
//     if (eachCategory._id.$oid === eachSC.categoryId.$oid) {
//       tranformedSubCategories.push(eachSC);
//     }
//   }
//   tranformedCategoriesSecondLevel.name = eachCategory.name;
//   tranformedCategoriesSecondLevel.description = eachCategory.description;
//   tranformedCategoriesSecondLevel.subcategories = tranformedSubCategories;
//   second_level_result.push(tranformedCategoriesSecondLevel);
// }
// console.log("second_level_result : " + JSON.stringify(second_level_result));
